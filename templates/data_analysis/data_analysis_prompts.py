# -*- coding: utf-8 -*-
"""
데이터 분석 프롬프트 V2.3 - Actionability 9.0 목표

핵심 개선: 실행가능성(Actionability) 8.16 → 9.0 목표
- Few-shot 예시: 좋은 실행 계획이 무엇인지 구체적 예시 제공
- 실행가능성 자기검증: LLM이 스스로 실행가능성 체크
- 정량적 수치 강제: 반드시 숫자/기한/담당자 포함
...
- [ ] 각 액션에 **구체적인 숫자**(%, 원, 건수)가 포함되어 있는가?
- [ ] **담당 부서/역할**이 명시되어 있는가?
- [ ] **구체적인 기한**(D+3, 1주 내 등)이 있는가?
- [ ] **실행 단계**가 1) → 2) → 3) 형태로 구체화되어 있는가?
- [ ] 내일 당장 시작할 수 있을 만큼 **구체적**인가?

**좋은 예시**:
> "마케팅팀이 D+3까지 VIP 고객(상위 10%, 523명) 대상 재구매 프로모션 이메일 발송.
> 실행: 1) 고객 리스트 추출 → 2) 이메일 템플릿 작성 → 3) A/B 테스트 발송 → 4) 성과 측정.
> 기대 효과: 재구매율 15%p 상승, 월 매출 3,200만원 증가 예상."

**나쁜 예시**:
> "고객 분석을 강화하고 마케팅 전략을 개선해야 합니다." (구체성 부족)
"""
...
**좋은 예시**:
> "운영팀이 D+7까지 이탈 위험 고객(이탈 확률 80% 이상, 127명) 대상 리텐션 콜 실행.
> 실행: 1) 대상 리스트 추출 → 2) 스크립트 준비 → 3) 콜 진행 → 4) 결과 기록.
> 기대 효과: 이탈률 25%p 감소, 월 LTV 1,500만원 보전."

**나쁜 예시**:
> "고객 이탈을 줄이기 위한 전략을 수립해야 합니다." (구체성 부족)
"""
...
아래 질문에 모두 "예"로 답할 수 있어야 합니다:
- [ ] 구현 **소요 시간**이 구체적으로 명시되어 있는가?
- [ ] **도구/환경**이 구체적으로 지정되어 있는가?
- [ ] **담당자**가 명시되어 있는가?
- [ ] **실행 단계**가 1) → 2) → 3) → 4) 형태로 구체화되어 있는가?
- [ ] 내일 당장 시작할 수 있을 만큼 **구체적**인가?

**좋은 예시**:
> "BI팀이 D+5까지 Tableau로 매출 대시보드 초안 완성.
> 실행: 1) 데이터 연결(30분) → 2) KPI 차트 3개 생성(1시간) → 3) 필터 설정(30분) → 4) 검토 요청.
> 완료 기준: 경영진이 3초 안에 핵심 지표를 파악할 수 있음."

**나쁜 예시**:
> "대시보드를 만들어서 경영진에게 공유하면 좋겠습니다." (구체성 부족)
"""
...
아래 질문에 모두 "예"로 답할 수 있어야 합니다:
- [ ] **구현 로드맵**에 구체적인 주차별 마일스톤이 있는가?
- [ ] 각 단계의 **담당자와 산출물**이 명시되어 있는가?
- [ ] **완료 기준**이 측정 가능한가?
- [ ] **알림 조건**에 구체적인 임계값이 있는가?
- [ ] 내일 당장 시작할 수 있을 만큼 **구체적**인가?

**좋은 예시**:
> "BI팀이 4주 내 영업 대시보드 구축 완료.
> 1주: 데이터 모델링 완료 → 2주: 차트 개발 → 3주: 사용자 테스트(영업팀 5명) → 4주: 배포.
> 알림: 월 매출 목표 90% 미달 시 영업팀장에게 Slack 알림.
> 완료 기준: 영업팀이 매일 아침 5분 내에 성과 현황 파악 가능."

**나쁜 예시**:
> "대시보드를 만들어서 영업팀에서 사용하도록 합니다." (구체성 부족)
"""
